defmodule AOCDay20 do
  def parse_module(s) do
    ...
    {name,
     %{type: type, children: children, parents: [], state: if(type == "&", do: %{}, else: 0)}}
  end
  ...
  def send_pulse(modules, queue, name, pulse) do
    case modules[name].type do
      "B" ->
        propagate(queue, modules[name].children, 0, name)
      ...
      "&" ->
        propagate(
          ...
          if(Enum.all?(Map.values(modules[name].state), &(&1 == 1)), do: 0, else: 1),
          name
        )
    end
  end
  ...
  def simulate(modules, queue, {l, h}, b) do
    ...
    if Map.has_key?(modules, name) do
      modules =
        if pulse == 0,
          do: Map.put(modules, name, Map.put_new(modules[name], :first, b)),
          else: modules
  ...
  def part1(file) do
    ...
    {_, {l, h}} =
      Enum.reduce(1..1000, {modules, {0, 0}}, fn b, {modules, counts} ->
  ...
  def part2(file) do
    ...
    {modules, _} =
      Enum.reduce(1..100_000, {modules, {0, 0}}, fn b, {modules, counts} ->
        simulate(modules, :queue.from_list([{"broadcaster", 0, "button"}]), counts, b)
      end)
