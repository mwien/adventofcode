defmodule AOCDay12 do
  ...
  def rec_solve({i, j, k}, springs, gaps, dp) do
    cond do
      Map.has_key?(dp, {i, j, k}) -> {dp, dp[{i, j, k}]}
      i == 0 and j == 0 and k == 0 -> {dp, 1}
      i < 0 or j < 0 or k < 0 -> {dp, 0} 
      k > 0 and springs[i] != ?. -> rec_solve({i-1, j-1, k-1}, springs, gaps, dp) 
      k == 0 and springs[i] != ?# -> 
        {dp1, sol1} = rec_solve({i-1, j, 0}, springs, gaps, dp) 
        {dp2, sol2} = if Map.has_key?(gaps, j), do: rec_solve({i-1, j, gaps[j]}, springs, gaps, dp1), else: {dp1, 0}
        {dp2, sol1 + sol2}
      true -> {dp, 0}
    end
  ...
  def modify_row(row) do
    ...
    new_springs = List.duplicate(springs, 5)
    |> Enum.join("?")
    new_seq = List.duplicate(seq, 5)
    |> Enum.join(",")
    [new_springs, new_seq]
  end
