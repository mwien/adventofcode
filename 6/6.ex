defmodule AOCDay6 do
  ...
  def parse(file, task) do
    ["Time:" <> times, "Distance:" <> distances] = File.read!(file)
    |> preprocess(task)
    |> String.trim()
    |> String.split("\n")
    Enum.zip(Enum.map(times |> String.trim() |> String.split(), &String.to_integer(&1)), Enum.map(distances |> String.trim() |> String.split(), &String.to_integer(&1)))
  end
