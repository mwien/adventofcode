defmodule AOCDay19 do
  ...
  def part1(file) do
    [workflows, parts] = File.read!(file)
    |> String.split("\n\n", trim: true)
  ...
  def next_poss({cat, op, val, next}, ranges) do
    ...
    cond do
      (val > b and op == "<") or (val < a and op == ">") -> [{next, ranges}]
      (val < a and op == "<") or (val > b and op == ">") -> [{nil, ranges}]
      op == "<" -> [{next, Map.put(ranges, cat, a..val-1)}, {nil, Map.put(ranges, cat, val..b)}]
      op == ">" -> [{nil, Map.put(ranges, cat, a..val)}, {next, Map.put(ranges, cat, val+1..b)}]
    end
  end
    ...
    |> Enum.map(fn {next, nranges} -> 
      case next do
        "A" -> 
          ...
          |> Enum.product()
        "R" -> 0
        nil -> count_combs(workflows, nranges, tail)
        next -> count_combs(workflows, nranges, Map.get(workflows, next))
      end
    end)
    |> Enum.sum()
  end
  ...
  def part2(file) do
    [workflows, _] = File.read!(file)
    |> String.split("\n\n", trim: true)
    
    workflows = parse_workflows(workflows)
    
    count_combs(workflows, Enum.map(["x", "m", "a", "s"], &({&1, 1..4000})) |> Map.new(), Map.get(workflows, "in")) 
  end
